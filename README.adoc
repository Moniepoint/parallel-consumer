...
This project is a continuation of https://github.com/confluentinc/parallel-consumer
...
[source,xml,indent=0]
        <dependency>
            <groupId>com.moniepoint.parallelconsumer</groupId>
...
Similar to ordered by key, this mode was created with batching in mind. It creates batches of a single key.

Only a single batch of any key will be processed at any given time.
If a key is received with another batch of the same key is in flight, that record will be stored in memory until it can be processed.

The option stackBatches (default = true) allows to join smaller batches together. If disabled, each batch will have a single key.
The BatchClassifier function allows to dynamically define if a record key needs to be unique in flight. If not, it will be treated as if UNORDERED
